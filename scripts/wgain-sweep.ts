...
import { JANELIA_TRACK_G } from "../src/lib/brain/corpus";
...
const CONFIGS: [number, number, number, number][] = [
  [1.4, 1900, 0.12, 40], // wire shipped hum, IDM opening record
  [1.4, 2500, 0.12, 40], // wire headroom
  [1.4, 1600, 0.12, 40], // janelia shipped hum (corpus swapped below)
];
for (const [WG, HUM, LEAK, STIM] of CONFIGS) {
  const isJane = HUM === 1600;
  const wBuf2 = isJane ? fs.readFileSync("public/data/flywire-weights-janelia.bin") : wBuf;
  const { w: w2 } = parseWeights(wBuf2.buffer.slice(wBuf2.byteOffset, wBuf2.byteOffset + wBuf2.byteLength) as ArrayBuffer);
  const brain = buildFlywireBrain(topo, w2, { seed: isJane ? 47 : 11, learning: false, motorElevation: 0.15, wGain: WG, leak: LEAK });
  const CORPUS = isJane ? JANELIA_TRACK_G : FLYWIRE_TRACK_B;
  let improv = 7;
  ...
  for (let phase = 0; phase < 2; phase++) {
    ...
    for (let step = 0; step < 32; step++) {
      for (let ch = 0; ch < 4; ch++) {
        const on = CORPUS.onsets[ch].includes(step);
      ...
      if (phase === 1) {
        ...
        let onStep = false;
        for (let ch = 0; ch < 4; ch++) if (CORPUS.onsets[ch].includes(step)) onStep = true;
